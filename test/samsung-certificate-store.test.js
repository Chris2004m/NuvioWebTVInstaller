...
test("recognizes Samsung certificate rejection variants", () => {
  assert.equal(certificateStore.isSamsungCertificateRejection("install failed[118]"), true);
  assert.equal(certificateStore.isSamsungCertificateRejection("install failed [118, -11]"), true);
...
test("keeps platform incompatibility separate from certificate rejection", () => {
  assert.equal(certificateStore.isSamsungPlatformIncompatibility("install failed[118019]"), true);
  assert.equal(certificateStore.isSamsungPlatformIncompatibility("install failed [118, -19]"), true);
  assert.equal(certificateStore.isSamsungCertificateRejection("install failed[118019]"), false);
  assert.equal(certificateStore.isSamsungCertificateRejection("install failed [118, -19]"), false);
});
